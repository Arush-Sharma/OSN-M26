...
int execute_builtin(Token *tokens, int token_count) {
    ...
    for (int i = 1; i < token_count; i++) {
        cmd_args[i - 1] = tokens[i].value;
    }
    int cmd_arg_count = token_count - 1;

    if (strcmp(cmd, "locate") == 0) {
        execute_locate(cmd_args, cmd_arg_count);
        return 1;
    }

    if (strcmp(cmd, "reveal") == 0) {
        execute_reveal(cmd_args, cmd_arg_count);
        return 1;
    }

    if (strcmp(cmd, "peek") == 0) {
        execute_peek(cmd_args, cmd_arg_count);
        return 1;
    }
