...
void scheduler(void)
{
  ...
  for(;;){
    ...
    // Strict priority selection (0 is highest)
    for(int q = 0; q < 4; q++) {
      for(p = proc; p < &proc[NPROC]; p++) {
        acquire(&p->lock);
        if(p->state == RUNNABLE && p->queue_level == q) {
          // Pick the one that has been waiting the longest (tail insertion)
          if (!best_p || p->enter_time < best_p->enter_time) {
             if (best_p) release(&best_p->lock);
             best_p = p;
             continue; // Keep lock on the new best_p
          }
        }
        release(&p->lock);
      }
      if (best_p) break; // Found a process in this high priority queue
    }
...
#ifdef MLFQ
  int max_ticks = (p->queue_level == 0) ? 1 :
                  (p->queue_level == 1) ? 4 :
                  (p->queue_level == 2) ? 8 : 16;
                  
  // Demote if time slice exhausted
  if (p->ticks_consumed >= max_ticks) {
      if (p->queue_level < 3) p->queue_level++;
      p->ticks_consumed = 0;
  }
  // Re-insert at the tail of the queue
  p->enter_time = ticks; 
#endif
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
    ...
    if (p->chan == chan) {
      ...
      if (p->state == SLEEPING) {
        p->state = RUNNABLE;
      #ifdef MLFQ
          p->enter_time = ticks; // Voluntary yield returns to tail of same queue
      #endif
      }
    }
    release(&p->lock);
...
#ifdef MLFQ
    printk("%d %s %s \t Q: %d \t Ticks: %d", p->pid, state, p->name, p->queue_level, p->ticks_consumed);
#else
