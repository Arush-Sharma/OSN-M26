...
usertrap(void)
{
  ...
  if(which_dev == 2 && myproc() != 0 && myproc()->state == RUNNING) {
      ...
      int max_ticks = (myproc()->queue_level == 0) ? 1 :
                      (myproc()->queue_level == 1) ? 4 :
                      (myproc()->queue_level == 2) ? 8 : 16;
                      
      int preempt = 0;
      if (myproc()->ticks_consumed >= max_ticks) {
          preempt = 1; // Time slice exhausted
      } else {
          // Strict priority preemption check
...
clockintr()
{
  if (cpuid() == 0) {
    ...
    #ifdef MLFQ
  // Priority boosting (anti-starvation)
  if (ticks % 48 == 0) {
    struct proc *p;
